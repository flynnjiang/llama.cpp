...
struct ggml_backend_kompute_context {
    ...
    kp::Manager manager;    // per mgr for per device
...
{
    return static_cast<ggml_backend_kompute_context *>(kompute_backend(device)->context);
}

static ggml_backend_kompute_context *kompute_backend_context(ggml_backend_buffer_t buffer)
{
    return static_cast<ggml_backend_kompute_context *>(kompute_backend(buffer)->context);
}

static ggml_backend_kompute_context *kompute_backend_context(ggml_backend_buffer_type_t buffer_type)
{
    return static_cast<ggml_backend_kompute_context *>(kompute_backend(buffer_type)->context);
}
...
bool ggml_vk_has_device() {
    return komputeManager.get_kp_manager()->hasDevice();
}

ggml_vk_device ggml_vk_current_device(struct ggml_backend_kompute_context *ctx) {
...
static void ggml_backend_kompute_buffer_free_buffer(ggml_backend_buffer_t buffer) {
    ...
    if (ggml_vk_has_device()) {
        ggml_vk_free_memory(backend_ctx, *memory);
    }
    delete memory;
}
...
static void ggml_backend_kompute_buffer_clear(ggml_backend_buffer_t buffer, uint8_t value) {
    ...
    if (memory->stagingBuffer)
        backend_ctx->manager.sequence()->eval<kp::OpBufferSyncDevice>(memory->primaryBuffer, memory->stagingBuffer, memory->size);
}
...
static ggml_backend_buffer_t ggml_backend_kompute_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buft, size_t size) {
    ...
    auto * ctx = new ggml_vk_memory(ggml_vk_allocate(backend_ctx, size));
...
{
    if (device >= 0 && device < m_backends.size())
